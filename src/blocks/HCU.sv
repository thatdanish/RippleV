...
typedef enum bit[2:0] { STALL_OFF, STALL_FOR_CJ, STALL_FOR_UCJ, STALL_FOR_RBW } state_t;
        ...
        STALL_OFF :  begin
            if ( bl_take_branch_i == 1'b1 ) 
                ...
                case (hcu_inst_type_i)
                    ...
                    HCU_CSR_type: if (csr_hazard(rs1_i)) next_state <= STALL_FOR_RBW; // stall ID & IF


                    // ********************************************* NOT DEBUGGED FROM HERE ************************************************** //
                    HCU_ecall: begin
                        // Increment counter
                        trap_stall_counter <= ( trap_stall_counter == TRAP_STALL_MAX ) ? trap_stall_counter : trap_stall_counter + 'd1;
            
                        
                        next_state <= (trap_stall_counter == TRAP_STALL_MAX) ? stall_off : handle_trap;
                    ...
                    HCU_mret: begin
                        ...
                        if ( mret_stall_counter == MRET_STALL_MAX ) begin
                            next_state <= stall_off; // clear stall after delay
                        end else
                            next_state <= stall_clear_UCJ; // stall 
                    ...
                    HCU_wfi: begin
                        ...
                        next_state <= stall_all; // stall complete cord

                    end
                    HCU_trap: begin
                        // Increment counter
                        trap_stall_counter <= ( trap_stall_counter == TRAP_STALL_MAX ) ? trap_stall_counter : trap_stall_counter + 'd1;
                        next_state <= (trap_stall_counter == TRAP_STALL_MAX) ? stall_off : handle_trapd
