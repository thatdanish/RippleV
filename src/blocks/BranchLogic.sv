  ...
  always_comb begin
    ...
    case (opr_i)
      ALU_JAL: begin
        ...
        jump_valid_o    = 1'b1;
        int_pc_jump     = func_jal(pc_i, sign_ext_offset_i);
      end
      ALU_JALR: begin
        int_take_branch = 1'b0;
        jump_valid_o    = 1'b1;
        int_pc_jump     = func_jalr(rs1_i, sign_ext_offset_i);
      end
      ALU_BEQ: begin
        int_take_branch = (rs2_i == rs1_i);
        jump_valid_o    = 1'b1;
        int_pc_jump     = signed'(pc_i) + signed'(sign_ext_offset_i);
      end
      ALU_BNE: begin
        int_take_branch = (rs2_i != rs1_i);
        jump_valid_o    = 1'b1;
