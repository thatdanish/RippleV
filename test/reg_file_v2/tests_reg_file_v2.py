...
async def read_write_test(dut):
    ...
    for _ in range(MAX_TEST):
        ...
        if rw == write:
            ...
            try:
                assert (
                    LogicArray(reg_file[rs1_addr.to_unsigned()], 32) == value_rs1
                ) and (LogicArray(reg_file[rs2_addr.to_unsigned()], 32) == value_rs2)
            except:
                raise AssertionError(
                    "Invalid read\n"
...
def test_runner_reg_file_v2():
    ...
    runner.test(hdl_toplevel="RegFilev2", test_module="tests_reg_file_v2", waves=waves)
