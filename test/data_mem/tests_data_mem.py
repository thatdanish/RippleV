...
def data_mem_model(data=0, addr=0, transfer=0, load=0, READ=False):
        ...
        # hex_byte
            ...
            if (int(addr[1:0])) == 0:
                ...
                raise AssertionError(
                    f"Misaligned read for HEX_WORD --> byte lane : {addr[1:0]}, addr : {int(addr[31:2])}"
                )
        # word
        else:
            if int(addr[1:0]) != 0:
                raise AssertionError(
                    f"Misaligned read for WORD --> byte lane : {addr[1:0]}, addr : {int(addr[31:2])}"
                )
            else:
    ...
    # hex_byte
        ...
        if (int(addr[1:0])) == 0:
            ...
            raise AssertionError(
                f"Misaligned write for HEX_WORD --> byte lane : {addr[1:0]}, addr : {int(addr[31:2])}"
            )
    # word
    else:
        if int(addr[1:0]) != 0:
            raise AssertionError(
                f"Misaligned write for WORD --> byte lane : {addr[1:0]}, addr : {int(addr[31:2])}"
            )
        else:
...
async def smoke_test(dut):
    ...
    for _ in range(int(N_TESTS / 2)):
        ...
        expected_value = data_mem_model(
            transfer=transfer, addr=logic_addr, load=load_type, READ=True
        )
        try:
            assert expected_value == dut.data_o.value
        except:
            cocotb.log.info(
                f"Address : {addr}, actual address : {addr>>2}, transfer : {transfer},  addr[1:0] : {addr%4}, load : {load_type}"
            )
            cocotb.log.info(
                f"Expected data mem : {LogicArray.from_unsigned(data_mem_dict[addr>>2], 32)}"
            )
            cocotb.log.info(f"Hardware data mem : {dut.dmem[addr>>2].value}")
            raise AssertionError(
                f"Invalid read --> ADDR {addr} expected : {expected_value}, got : {dut.data_o.value}"
            )
...
def test_runner_data_mem():
    ...
    runner.test(hdl_toplevel="data_mem", test_module="tests_data_mem", waves=waves)
