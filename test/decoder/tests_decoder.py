    ...
    for _ in range(N_TESTS):
        ...
        try:
            ...
            raise AssertionError(
                f"expected Rd : {''.join(reversed(inst_bin[7:12]))}, got : {dut.rd_o.value}"
            )
        try:
            assert dut.rs1_o.value == "".join(reversed(inst_bin[15:20]))
        except:
            raise AssertionError(
                f"expected Rs1 : {''.join(reversed(inst_bin[15:20]))}, got : {dut.rs1_o.value}"
            )
        try:
            assert dut.rs2_o.value == "".join(reversed(inst_bin[20:25]))
        except:
            raise AssertionError(
                f"expected Rs2 : {''.join(reversed(inst_bin[20:25]))}, got : {dut.rs2_o.value}"
            )
        try:
            assert dut.lui_o.value[31:12] == "".join(reversed(inst_bin[12:32]))
        except:
            raise AssertionError(
                f"expected LUI [12:31] : {''.join(reversed(inst_bin[12:32]))}, got : {dut.lui_o.value[31:12]}"
