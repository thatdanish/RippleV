...
async def monitor(dut):
    while True:
        ...
        if (
            (dut.data_mem_inst.dmem[TO_HOST].value == SUCCESS)
            or (dut.data_mem_inst.dmem[TO_HOST_RVT].value == SUCCESS_RVT)
            or (dut.data_mem_inst.dmem[TO_HOST_RVT_2].value == SUCCESS_RVT)
        ):
            cocotb.pass_test()
...
async def run_test(dut):
    ...
    try:
        ...
        raise AssertionError(
            f"Invalid RST_HND : expected : {RST_HND}, got : {dut.pc_final.value}"
        )
    ...
    raise AssertionError(
        f"Incorrect TO_HOST ({hex(TO_HOST)}) read --> exp : {hex(SUCCESS)}, got : {hex(dut.data_mem_inst.dmem[TO_HOST].value)}, OR\n"
